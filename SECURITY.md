...
The global wipe's simultaneous lock order is exactly `snapshot cache -> ledger map -> collection timestamp`. Normal snapshot generation does not hold these three simultaneously: it projects ledger data under its lock and processes it afterward.
