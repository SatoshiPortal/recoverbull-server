...
python3 deploy/backup/sqlite_backup.py verify /var/backups/recoverbull/database.sqlite3
```
...
Before a restore, perform a controlled drill on a new path and run `verify`; do not infer RPO or RTO unless the operator has configured and measured them. The selected proxy service is deployment-specific; substitute its actual systemd unit for `<proxy-service>` below and do not stop an unrelated proxy.
...
3. Verify and restore the selected backup to the now-empty configured path: `python3 deploy/backup/sqlite_backup.py verify /var/backups/recoverbull/database.sqlite3` then `python3 deploy/backup/sqlite_backup.py restore /var/backups/recoverbull/database.sqlite3 /var/lib/recoverbull/database.sqlite3`.
