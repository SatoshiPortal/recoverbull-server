...
// Keep the security-sensitive network stack explicit. This matches the
// version selected by the module graph instead of downgrading it; go.sum,
// `go mod verify`, the readonly build and the govulncheck CI gate together
// pin and continuously audit the exact source used by the reference proxy.
