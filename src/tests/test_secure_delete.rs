...
fn test_application_connection_enables_secure_delete() {
    ...
    let mut connection = crate::database::establish_connection(state.database_url).unwrap();
...
fn test_application_connection_uses_patched_sqlite_and_wal() {
    ...
    let version = diesel::sql_query("SELECT sqlite_version() AS value")
        ...
        .unwrap();
    assert!(crate::database::sqlite_version_at_least_for_test(
        &version.value
    ));
...
fn test_sqlite_version_comparator_boundaries() {
    assert!(!crate::database::sqlite_version_at_least_for_test("3.51.2"));
    assert!(crate::database::sqlite_version_at_least_for_test("3.51.3"));
    assert!(crate::database::sqlite_version_at_least_for_test("3.52.0"));
    assert!(!crate::database::sqlite_version_at_least_for_test(
        "not-a-version"
    ));
}
...
fn test_memory_database_fails_closed_when_wal_is_unavailable() {
    assert_eq!(
        crate::database::establish_connection(":memory:".to_owned())
            .err()
            .expect("in-memory SQLite must fail the WAL check"),
        crate::database::ConnectionSetupError::Wal
    );
}
