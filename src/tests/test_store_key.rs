...
async fn test_failure_backup_key_duplicated() {
    ...
    server.post(&"/key").json(key).expect_success().await;
    let duplicate = server.post(&"/key").json(key).expect_failure().await;
...
async fn test_failure_secret_not_64_letters() {
    ...
    let response = server
        ...
        .json(&crate::models::StoreKey {
            ...
            secret_hash: "not-64-chars".to_string(),
        })
        .expect_failure()
        .await;
...
async fn test_failure_secret_not_base16_chars() {
    ...
    let response = server
        ...
        .json(&crate::models::StoreKey {
            backup_key: "random".to_string(),
            secret_hash: "zd969eef6ecad3c29a3a629280e686cf0c3f5d5a86aff3ca12020c923adc6c9z"
                .to_string(),
        })
        .expect_failure()
        .await;
